...
impl MidiInput {
        ...
        F: FnMut(u64, &[u8], &mut T) + Send + 'static,
    {
        ...
        sub.set_sender(Addr {
            client: src_pinfo.get_client(),
            port: src_pinfo.get_port(),
        });
        sub.set_dest(Addr {
...
fn handle_input<T>(mut data: HandlerData<T>, user_data: &mut T) -> HandlerData<T> {
...

    {
        ...
        while do_input {
            ...
            use backend::alsa::nix::errno::Errno::EAGAIN;
            use backend::alsa::nix::errno::Errno::ENOSPC;
            let mut ev = match seq_input.event_input() {
                Ok(ev) => ev,
                Err(ref e) if e.errno() == Some(ENOSPC) => {
                    let _ = writeln!(
                        stderr(),
                ...
                Err(ref e) if e.errno() == Some(EAGAIN) => {
                    let _ = writeln!(
                        stderr(),
